...
interface externalInterface is IERC1155{
    ...
    function setFractionPrice(uint256 tokenId, uint256 price) external;
...
contract PoulinaMarketplace is ERC1155Holder{
    ...
    constructor(address _PoulinaTokenAddr) {
      Poulina = externalInterface(_PoulinaTokenAddr);
    ...
    function createSale(
        ...
        require(
            Poulina.balanceOf(msg.sender, tokenId) >= amount,
            "Insufficient token to create sale"
        );
        Poulina.proxyTransferFrom(msg.sender, address(this), tokenId, amount, "0x0000");
        uint256 i;
        uint256 length = tokenIdToSales[tokenId].length;
        uint256 price = Poulina.fractionPrice(tokenId);
        for (
            i = 0;
            i < length &&
    ...
    function cancelSale(
        ...
        Poulina.proxyTransferFrom(address(this), msg.sender, tokenId, amount, "0x0000");
        ...
        )
    {
        uint256 length;
        uint256 i;
        for (i = 1; i <= Poulina.getTotalMinted(); ++i) {
            if (Poulina.exists(i)) {
